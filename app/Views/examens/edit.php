    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="<?= $action ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fichier de l'examen (PDF ou Image)</label>
                    
                    <?php if (!empty($examen['fichier_sujet'])): ?>
                        <div class="mb-2 flex items-center justify-between p-3 bg-indigo-50 border border-indigo-100 rounded-md">
                            <div class="flex items-center gap-3">
                                <i class="fas fa-file-alt text-indigo-600"></i>
                                <span class="text-sm font-medium text-indigo-900">Fichier actuel présent</span>
                            </div>
                            <a href="<?= url($examen['fichier_sujet']) ?>" target="_blank" class="text-xs text-indigo-700 hover:underline">
                                Voir le fichier
                            </a>
                        </div>
                    <?php endif; ?>
                        ...
                        <div class="space-y-1 text-center">
                            <div class="flex text-sm text-gray-600">
                                <label for="fichier_sujet" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                    <span><?= !empty($examen['fichier_sujet']) ? 'Remplacer le fichier' : 'Télécharger un fichier' ?></span>
                                    <input id="fichier_sujet" name="fichier_sujet" type="file" class="sr-only" accept=".pdf,image/*" <?= empty($examen['fichier_sujet']) ? 'required' : '' ?>>
                                </label>

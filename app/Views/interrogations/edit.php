    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="<?= $action ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <!-- Fichier Sujet (Obligatoire) -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fichier de l'épreuve (PDF ou Image)</label>
                    
                    <?php if (!empty($interrogation['fichier_sujet'])): ?>
                        <div class="mb-2 flex items-center justify-between p-3 bg-purple-50 border border-purple-100 rounded-md">
                            <div class="flex items-center gap-3">
                                <i class="fas fa-file-alt text-purple-600"></i>
                                <span class="text-sm font-medium text-purple-900">Fichier actuel présent</span>
                            </div>
                            <a href="<?= url($interrogation['fichier_sujet']) ?>" target="_blank" class="text-xs text-purple-700 hover:underline">
                                Voir le fichier
                            </a>
                        </div>
                    <?php endif; ?>
                        ...
                        <div class="space-y-1 text-center">
                            <div class="flex text-sm text-gray-600">
                                <label for="fichier_sujet" class="relative cursor-pointer bg-white rounded-md font-medium text-purple-600 hover:text-purple-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-purple-500">
                                    <span><?= !empty($interrogation['fichier_sujet']) ? 'Remplacer le fichier' : 'Télécharger un fichier' ?></span>
                                    <input id="fichier_sujet" name="fichier_sujet" type="file" class="sr-only" accept=".pdf,image/*" <?= empty($interrogation['fichier_sujet']) ? 'required' : '' ?>>
                                </label>
                                <p class="pl-1">ou glisser-déposer</p>
                            </div>
                            <p class="text-xs text-gray-500">PDF, PNG, JPG jusqu'à 10MB</p>
                            <?php if (!empty($interrogation['fichier_sujet'])): ?>
                                <p class="text-xs text-orange-500 mt-2">Laisser vide pour conserver le fichier actuel</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>

<?php $title = "Détails enseignant - " . htmlspecialchars($enseignant['nom'] . ' ' . $enseignant['prenom']); ?>
...
<div class="mt-6 mb-8 px-6 flex justify-between items-center">
    <div class="flex items-center gap-4">
        <h1 class="text-3xl font-bold text-gray-800">
            ...
            <span class="text-sm font-normal text-gray-500 ml-3">(<?= htmlspecialchars($enseignant['matricule']) ?>)</span>
        </h1>
    </div>
    <div class="flex gap-3">
        ...
        <a href="<?= url('enseignants/edit/' . $enseignant['id']) ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm p-6 text-center">
            <?php if (!empty($enseignant['photo'])): ?>
                <img src="<?= public_url($enseignant['photo']) ?>" alt="Photo" class="w-32 h-32 rounded-full mx-auto object-cover border-4 border-gray-100 mb-4">
            <?php else: ?>
            ...
            <h2 class="text-xl font-semibold text-gray-800"><?= htmlspecialchars($enseignant['prenom']) ?></h2>
            <p class="text-gray-500 uppercase text-sm font-bold tracking-wide"><?= htmlspecialchars($enseignant['nom']) ?></p>
            <?php if (!empty($enseignant['specialite'])): ?>
            <p class="mt-2 text-indigo-600 font-medium"><?= htmlspecialchars($enseignant['specialite']) ?></p>
            <?php endif; ?>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 space-y-2 text-left">
                <div class="flex items-center gap-2 text-gray-600">
                    ...
                    <span class="text-sm"><?= htmlspecialchars($enseignant['telephone']) ?></span>
                </div>
                <div class="flex items-center gap-2 text-gray-600">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    <span class="text-sm truncate"><?= htmlspecialchars($enseignant['email']) ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-3">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="{ tab: 'info' }">
            <div class="border-b border-gray-200 flex flex-wrap">
                <button @click="tab = 'info'" :class="{'border-blue-600 text-blue-600': tab === 'info', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'info'}" class="flex-1 py-4 px-4 text-center border-b-2 font-medium transition-colors">Informations</button>
                <button @click="tab = 'pedago'" :class="{'border-blue-600 text-blue-600': tab === 'pedago', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'pedago'}" class="flex-1 py-4 px-4 text-center border-b-2 font-medium transition-colors">Pédagogie</button>
                <button @click="tab = 'docs'" :class="{'border-blue-600 text-blue-600': tab === 'docs', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'docs'}" class="flex-1 py-4 px-4 text-center border-b-2 font-medium transition-colors">Documents</button>
                <button @click="tab = 'absences'" :class="{'border-blue-600 text-blue-600': tab === 'absences', 'border-transparent text-gray-500 hover:text-gray-700': tab !== 'absences'}" class="flex-1 py-4 px-4 text-center border-b-2 font-medium transition-colors">Absences</button>
            </div>

            <!-- Tab: Informations -->
            <div x-show="tab === 'info'" class="p-6">
                <!-- Même contenu que Personnel mais avec champs Enseignant -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Spécialité</h3>
                        <p class="text-gray-900"><?= htmlspecialchars($enseignant['specialite'] ?? 'Non renseigné') ?></p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Diplôme</h3>
                        <p class="text-gray-900"><?= htmlspecialchars($enseignant['diplome']) ?></p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Grade</h3>
                        <p class="text-gray-900"><?= htmlspecialchars($enseignant['grade'] ?? 'Non renseigné') ?></p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Expérience</h3>
                        <p class="text-gray-900"><?= htmlspecialchars($enseignant['anciennete_annees'] ?? '0') ?> ans</p>
                    </div>
                    <div class="md:col-span-2 border-t border-gray-100 pt-4 mt-2">
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Infos Personnelles</h3>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Date de naissance</h3>
                        <p class="text-gray-900"><?= date('d/m/Y', strtotime($enseignant['date_naissance'])) ?></p>
                        <p class="text-sm text-gray-500">à <?= htmlspecialchars($enseignant['lieu_naissance']) ?></p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 mb-1">CIN</h3>
                        <p class="text-gray-900"><?= htmlspecialchars($enseignant['cin']) ?></p>
                    </div>
                    <div class="md:col-span-2">
                        <h3 class="text-sm font-medium text-gray-500 mb-1">Adresse</h3>
                        <p class="text-gray-900"><?= nl2br(htmlspecialchars($enseignant['adresse'])) ?></p>
                    </div>
                </div>
            </div>

            <!-- Tab: Pédagogie -->
            <div x-show="tab === 'pedago'" class="p-6" style="display: none;">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php if (!empty($matieres)): ?>
                    <div class="bg-gray-50 rounded-lg p-4">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                            <i class="fas fa-book text-indigo-600"></i>
                            Matières enseignés (<?= count($matieres) ?>)
                        </h3>
                        <div class="space-y-2">
                            <?php foreach ($matieres as $matiere): ?>
                                <div class="bg-white border border-gray-200 rounded p-3 shadow-sm">
                                    <p class="font-semibold text-gray-800"><?= htmlspecialchars($matiere['nom']) ?></p>
                                    <?php if (!empty($matiere['code'])): ?>
                                        <p class="text-sm text-gray-600">Code: <?= htmlspecialchars($matiere['code']) ?></p>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="space-y-2">
                            <?php foreach ($classes as $classe): ?>
                                <div class="bg-white border border-gray-200 rounded p-3 shadow-sm">
                                    <p class="font-semibold text-gray-800"><?= htmlspecialchars($classe['nom']) ?></p>
                                    <?php if (!empty($classe['matiere_nom'])): ?>
                                        <p class="text-sm text-gray-600">Matière: <?= htmlspecialchars($classe['matiere_nom']) ?></p>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <?php if (empty($matieres) && empty($classes)): ?>
                        <div class="col-span-2 text-center text-gray-500 py-8">
                            Aucune donnée pédagogique associée pour le moment.
                        </div>
                    <?php endif; ?>
                </div>
            </div>

            <!-- Tab: Documents (Link to PersonnelController) -->
            <div x-show="tab === 'docs'" class="p-6" style="display: none;">
                    ...
                    <ul class="divide-y divide-gray-100">
                        ...
                        <li class="py-3 flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-900"><?= htmlspecialchars($doc['type_document']) ?></p>
                                    <p class="text-xs text-gray-500"><?= date('d/m/Y', strtotime($doc['date_upload'])) ?></p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <a href="<?= public_url($doc['fichier']) ?>" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm">Voir</a>
                ...
                <dialog id="modal-add-doc" class="modal rounded-lg p-0 shadow-xl w-full max-w-md">
                    <form action="<?= url('personnel/addDocument/' . $enseignant['personnel_id']) ?>" method="POST" enctype="multipart/form-data" class="p-6">
                        <h3 class="font-bold text-lg mb-4">Nouveau document</h3>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Type</label>
                                <select name="type_document" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <!-- Tab: Absences (Link to PersonnelController) -->
            <div x-show="tab === 'absences'" class="p-6" style="display: none;">
                ...
                <dialog id="modal-add-abs" class="modal rounded-lg p-0 shadow-xl w-full max-w-md">
                    <form action="<?= url('personnel/addAbsence/' . $enseignant['personnel_id']) ?>" method="POST" enctype="multipart/form-data" class="p-6">

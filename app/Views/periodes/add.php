...
<div class="p-4 md:p-8">
    <div class="mb-4 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Pour l'année scolaire <strong><?= e($annee['libelle']) ?></strong></p>
        </div>
        <a href="<?= url('periodes/list?annee_id=' . $annee['id']) ?><?= !empty($_GET['iframe']) ? '&iframe=1' : '' ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition flex items-center gap-2 shadow-md font-medium">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
        <form method="POST" action="<?= url('periodes/add') ?><?= !empty($_GET['iframe']) ? '?iframe=1' : '' ?>" class="space-y-6">
            <?= csrf_field() ?>
            ...
            <div>
                ...
                <input type="text" name="nom" required 
                       placeholder="Ex: 1er Trimestre, 1er Semestre..."
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="numero" required min="1" max="10" value="1"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
            ...
            <div class="pt-6 border-t flex gap-3">
                ...
                <button type="submit" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow-lg transition">
                    <i class="fas fa-save mr-2"></i> Créer la période
                </button>
            </div>
        </form>
    </div>

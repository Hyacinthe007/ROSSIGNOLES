...
<div class="p-4 md:p-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-gray-500 text-sm mb-1">
                <a href="<?= url('classes/list') ?>?iframe=1" class="hover:text-blue-600 transition">Classes</a>
                <i class="fas fa-chevron-right text-xs"></i>
                <span>Coefficients par Classe</span>
            </div>
            <h1 class="text-lg md:text-xl font-semibold text-gray-800">
                <i class="fas fa-chalkboard text-blue-600 mr-2"></i>
                Matières & Coefficients : <?= e($classe['nom']) ?>
            </h1>
            <p class="text-xs text-gray-500 mt-1">
                Configuration spécifique pour l'année scolaire <?= e($classe['annee_scolaire'] ?? 'N/A') ?>
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-2">
            <form action="<?= url('pedagogie/classes/update-coefficients') ?><?= isset($_GET['iframe']) ? '?iframe=1' : '' ?>" method="POST" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
                <?= csrf_field() ?>
                ...
                <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h2 class="font-bold text-gray-700">Matières actives pour cette classe</h2>
                        <p class="text-xs text-amber-600 font-medium mt-1">
                            <i class="fas fa-info-circle mr-1"></i>
                            Ces réglages surchargent ceux de la série et du niveau.
                        </p>
                    </div>
                            ...
                            <?php if (empty($matieresAssociees)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                                        ...
                                        Aucune matière spécifique configurée pour cette classe.<br>
                                        Elle utilise par défaut les coefficients de sa série (<?= e($classe['serie_nom'] ?? 'N/A') ?>).
                                    </td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr class="hover:bg-blue-50/30 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <input type="number" 
                                                   ...
                                                   step="0.25" min="0.25"
                                                   class="w-24 px-3 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 outline-none font-medium">
                                        </td>
                                        <td class="px-6 py-4">
                                            <input type="number" 
                                                   ...
                                                   step="0.5" min="0"
                                                   class="w-24 px-3 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 outline-none">
                                        </td>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div class="relative">
                        <input type="text" id="matiereSearch" placeholder="Rechercher..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                    ...
                    <div class="max-h-[50vh] overflow-y-auto pr-2 space-y-2" id="matiereList">
                        ...
                        ?>
                            <div class="matiere-item p-3 border rounded-lg flex items-center justify-between hover:bg-gray-50 transition <?= $isAssoc ? 'opacity-50 pointer-events-none' : '' ?>" 
                                ...
                                <?php else: ?>
                                    <span class="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold uppercase">Ajoutée</span>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <div class="bg-amber-50 rounded-xl shadow-sm border border-amber-200 p-6">
                ...
                <p class="text-sm text-amber-900 leading-relaxed">
                    Définir des matières au niveau de la classe est utile pour les matières optionnelles ou si une classe particulière a des coefficients différents du reste de sa série. 
                    <br><br>
                    <strong>Si cette liste est vide</strong>, le bulletin utilisera automatiquement les réglages de la série.
                </p>
            </div>
        </div>
    </div>
...
function addMatiereToTable(id, nom, code) {
    ...
    row.innerHTML = `
        ...
        <td class="px-6 py-4">
            <input type="number" name="matieres[${id}][coefficient]" value="1" step="0.25" min="0.25" class="w-24 px-3 py-1 border border-blue-300 bg-blue-50 rounded focus:ring-2 focus:ring-blue-500 outline-none font-medium">
        </td>
        <td class="px-6 py-4">
            <input type="number" name="matieres[${id}][heures_semaine]" value="" step="0.5" min="0" placeholder="0" class="w-24 px-3 py-1 border border-blue-300 bg-blue-50 rounded focus:ring-2 focus:ring-blue-500 outline-none">
        </td>
        <td class="px-6 py-4 text-center">
            <label class="relative inline-flex items-center cursor-pointer">
...
function removeMatiere(btn) {
    if (!confirm('Voulez-vous retirer cette matière de la classe ?')) return;
    ...
    setTimeout(() => {
        ...
        // Remettre le message vide si besoin
        const tbody = document.querySelector('tbody');
        if (tbody.children.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-12 text-center text-gray-400"><i class="fas fa-book-open text-4xl mb-4 block opacity-20"></i>Aucune matière spécifique configurée pour cette classe.</td></tr>';
        }
    }, 300);
}
...
/* Forcer l'affichage des flèches (spin buttons) sur Chrome/Safari/Edge/Firefox */

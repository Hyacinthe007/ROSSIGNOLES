...
<div class="p-4 md:p-8">
    <div class="mb-4 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Configuration des coefficients par série</p>
        </div>
        <div class="flex gap-2">
            <a href="<?= url('pedagogie/series') ?><?= !empty($_GET['iframe']) ? '?iframe=1' : '' ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition flex items-center gap-2 shadow-md font-medium">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-gray-500 text-sm mb-1">
                <a href="<?= url('pedagogie/series') ?>?iframe=1" class="hover:text-blue-600 transition">Séries</a>
                <i class="fas fa-chevron-right text-xs"></i>
                <span>Coefficients</span>
            </div>
            <h1 class="text-lg md:text-xl font-semibold text-gray-800">
                <i class="fas fa-percentage text-blue-600 mr-2"></i>
                Coefficients : <?= e($serie['libelle'] ?? $serie['nom']) ?>
                <span class="ml-2 text-sm font-normal text-gray-500 bg-gray-100 px-2 py-1 rounded">
                    <?= e($serie['niveau_nom']) ?>
                </span>
            </h1>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-2">
            <form action="<?= url('pedagogie/series/update-coefficients') ?><?= isset($_GET['iframe']) ? '?iframe=1' : '' ?>" method="POST" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
                <?= csrf_field() ?>
                <input type="hidden" name="serie_id" value="<?= $serie['id'] ?>">

                <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h2 class="font-bold text-gray-700">Matières associées</h2>
                            ...
                            <?php if (empty($matieresAssociees)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                                        ...
                                        Aucune matière configurée pour cette série.<br>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-blue-600 rounded-xl shadow-lg p-6 text-white overflow-hidden relative">
                ...
                <p class="text-sm text-blue-100 leading-relaxed">
                    Les coefficients sont cruciaux pour le calcul automatique des moyennes. 
                    Un coefficient de 0.00 désactive la matière pour cette série.
                </p>
            </div>
        </div>
    </div>
...
function addMatiereToTable(id, nom, code) {
    ...
    // Supprimer le message "Aucune matière" si présent
    ...
    if (document.querySelector(`input[name="matieres[${id}][coefficient]"]`)) {
        return;
    }
    ...
    row.innerHTML = `
        ...
        <td class="px-6 py-4">
            <input type="number" 
                   name="matieres[${id}][coefficient]" 
                   value="1" 
                   step="0.25" min="0.25"
                   class="w-20 text-center px-2 py-1 border border-blue-300 bg-blue-50 rounded focus:ring-2 focus:ring-blue-500 outline-none">
        </td>
        <td class="px-6 py-4">
            <input type="number" 
                   name="matieres[${id}][heures_semaine]" 
                   value="" 
                   step="0.5" min="0"
                   placeholder="0"
                   class="w-20 text-center px-2 py-1 border border-blue-300 bg-blue-50 rounded focus:ring-2 focus:ring-blue-500 outline-none">
    ...
    // Griser l'item dans la liste de droite
...
function removeMatiere(btn) {
    if (!confirm('Voulez-vous retirer cette matière de la série ?')) return;
    ...
    setTimeout(() => {
        ...
        // Réactiver l'item dans la liste de droite
        const listItem = document.querySelector(`.matiere-item[data-id="${id}"]`);
        if (listItem) {
        ...
        // Si plus aucune matière, afficher le message
        const tbody = document.querySelector('tbody');
...
document.getElementById('matiereSearch').addEventListener('input', function(e) {
    ...
    document.querySelectorAll('.matiere-item').forEach(item => {
        ...
        if (nom.includes(text) || code.includes(text)) {

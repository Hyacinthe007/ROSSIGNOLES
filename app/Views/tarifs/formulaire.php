    ...
    <div class="mb-4 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">
                <?= isset($tarif) ? 'Modification du tarif #' . $tarif['id'] : 'CrÃ©ation d\'un nouveau tarif scolaire' ?>
            </p>
        </div>
        ...
        <form method="POST" action="<?= (isset($tarif) ? url('tarifs/mettre-a-jour/' . $tarif['id']) : url('tarifs/creer')) ?><?= !empty($_GET['iframe']) ? '?iframe=1' : '' ?>">
            ...
            <div class="mb-6">
                <label for="annee_scolaire_id" class="block text-sm font-medium text-gray-700 mb-2">
                    <i class="fas fa-calendar mr-2 text-gray-500"></i>AnnÃ©e Scolaire *
                </label>
                ...
                <select id="niveau_id" name="niveau_id" required
                    ...
                    <?php foreach ($niveaux as $niveau): ?>
                        <option value="<?= $niveau['id'] ?>"
                            ...
                            <?= e($niveau['libelle']) ?> (<?= e($niveau['cycle_libelle'] ?? 'Cycle non dÃ©fini') ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
                <p class="text-xs text-gray-500 mt-1">SÃ©lectionnez le niveau pour lequel appliquer ce tarif.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                     <label for="ecolage_mensuel" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-money-bill-wave mr-2 text-gray-500"></i>Ã‰colage Mensuel *
                    </label>
                    <div class="relative">
...
document.addEventListener('DOMContentLoaded', function() {
    // Fonction pour formater un nombre avec des espaces comme sÃ©parateurs de milliers
    function formatNumber(value) {
        // Enlever tous les espaces et caractÃ¨res non numÃ©riques sauf le point
    ...
    // Fonction pour obtenir la valeur numÃ©rique (sans espaces)
    function getNumericValue(value) {
        return value.replace(/\s/g, '');
    }

    // Appliquer le formatage Ã  tous les champs avec la classe amount-format

...
<body class="bg-gradient-to-br from-blue-100 via-blue-200 to-green-100 relative h-full w-full overflow-hidden">
  ...
  <!-- Background container (Fixé pour ne pas gêner le centrage) -->
  ...
  <div class="relative z-10 h-full w-full flex items-center justify-center p-4">
    ...
    <div class="text-center mb-6">
      <div class="text-6xl mb-4 transform hover:scale-110 transition-transform cursor-default inline-block">🎓</div>
      <h1 class="text-3xl font-bold text-blue-700">École Mandroso</h1>
    ...
    <div class="mt-8 text-center border-t border-gray-200 pt-4">
        <p class="text-xs text-gray-500">© <?= date('Y') ?> École Mandroso. Tous droits réservés.</p>
    </div>
    </div>
  </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Génération dynamique des icônes d'arrière-plan
        ...
        // Création basée sur une grille pour une distribution uniforme
        const cols = 10; // Augmenter le nombre de colonnes pour plus de granularité
        const rows = 8;
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                // Probabilité d'avoir une icône (plus élevée pour garantir la couverture)
                // On force un peu plus sur les bords (colonne 0 et dernière)
